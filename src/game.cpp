...
namespace utils
{
    ...
    {
        //  Output which player won!
        //  Not the endl and \t are for formatting purposes
        cls();
        cout << endl << endl << "\t\t\t /$$     /$$                        /$$      /$$ /$$            /$$	" << endl;
        cout << "\t\t\t|  $$   /$$/                       | $$  /$ | $$|__/           | $$	" << endl;
        cout << "\t\t\t \\  $$ /$$//$$$$$$  /$$   /$$      | $$ /$$$| $$ /$$ /$$$$$$$  | $$	" << endl;
        ...
        cout << "\t\t\t    | $$ | $$  | $$| $$  | $$      | $$$/ \\  $$$| $$| $$  | $$		" << endl;
        cout << "\t\t\t    | $$ |  $$$$$$/|  $$$$$$/      | $$/   \\  $$| $$| $$  | $$  /$$	" << endl;
        cout << "\t\t\t    |__/  \\______/  \\______/       |__/     \\__/|__/|__/  |__/ |__/	" << endl;

        //  Does P1 or P2 have the most hits?
        if(hits1 == maxHits)
    ...
    void isHit(int r, int c, string t)
    {
        ...
        //  If that statement is false, the board checks for a ship (isAlpha) and updates with an 'X' if it was a successful hit and increments hits for that player, otherwise update board with an 'O'
        if(t == p2)
        {
            if(p1OutputBoard[r][c] == 'X' || p1OutputBoard[r][c] == 'O')
            {
                //  Get new input
                cout << endl << "Hey dummy, you've already fired there!" << endl; 
                getCoords(p2);
            }
            else
            {
                if(isalpha(p1Board[r][c]) != 0)
                {
                    ...
                    p1OutputBoard[r][c] = 'X';
                    refresh();
                    cout << endl << p2 << ", hit or miss, I guess you never miss, huh? Hit!" << endl;
                }
                else
                {
                    //  Update display
                    p1OutputBoard[r][c] = 'O';
                    refresh();
                    cout << endl << "You know, " << p2 << ", you can always change the difficulty. Miss!" << endl;
                }
            }
        }
        else
            ...
            if(t == p1)
            {
                ...
                else
                {
                    if(isalpha(p2Board[r][c]) != 0)
                    {
                        ...
                        p2OutputBoard[r][c] = 'X';
                        refresh();
                        cout << endl << p1 << ", you're an absolute mad lad! Hit!" << endl;
                    }
                    else
                    {
                        //  Update display
                        p2OutputBoard[r][c] = 'O';
                        refresh();
                        cout << endl << p1 << ", your aim could use some improvement. Miss!" << endl;
                    }
                }
            }
        }     
